...
// ─── Static sub-components (defined OUTSIDE the parent to avoid remount on every render) ───
...
export default function ScreenLeaderboard() {
    ...
    // Effect 1: live score countdown – writes directly to DOM, no React re-render
    useEffect(() => {
    ...
    // Effect 2: overlay state machine – ref-based timer survives status changes
    useEffect(() => {
        ...
        // Game just ended: backend says 'finished', or status jumped FROM 'playing' directly
    ...
    // Memoize grandmaster section – only recomputes when grandmaster data changes
    const grandmasterSection = useMemo(() => (
        <div className="flex-none h-[45%] mb-6 relative z-10">
            <motion.div
                ...
                className="bg-surface/60 backdrop-blur-lg border-4 border-accent/50 rounded-2xl p-4 xl:p-8 shadow-[0_0_50px_rgba(243,234,95,0.1)] h-full flex flex-col relative overflow-hidden"
            >
    ...
    if (isLoading) return (
        <div className="p-10 text-center animate-pulse font-mono text-2xl h-screen flex items-center justify-center bg-black text-white">
    ...
    return (
        <div className="h-screen w-screen bg-black overflow-hidden p-4 xl:p-8 flex flex-col absolute top-0 left-0 right-0 bottom-0 z-50">
            ...
            {/* Live Patch Master Overlay – fixed so it's always viewport-centered */}
            <AnimatePresence>
                            ...
                            className={`bg-black/95 border-4 ${
                                isFinishedMode
                                    ? displayScore >= 5000
                                        ...
                                        : 'border-red-500 shadow-[0_0_150px_rgba(239,68,68,0.6)]'
                                    : 'border-accent shadow-[0_0_150px_rgba(243,234,95,0.4)]'
                        ...
                        >
                            {isFinishedMode ? (
                                ...
                                <>
                                    ...
                                    <div
                                        ref={scoreDisplayRef}
                                        className="text-[140px] leading-none font-black text-accent tracking-widest font-mono drop-shadow-[0_0_30px_rgba(243,234,95,0.8)]"
                                    >
