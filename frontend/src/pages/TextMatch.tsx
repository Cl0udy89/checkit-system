...
const GAME_DURATION_S = 90          // total game time (hidden)
const INITIAL_SCORE   = 10000
const WRONG_PENALTY   = 500
const DECAY_PER_S     = INITIAL_SCORE / GAME_DURATION_S   // ≈ 111 pts/s
const PARTICLE_COLORS = ['#4ade80','#86efac','#facc15','#a78bfa','#60a5fa','#f87171','#fb923c']

function makeBurst(x: number, y: number, n = 12): Particle[] {
...
export default function TextMatch() {
    ...
    const visibleTerms = termOrder.filter(id => !matched.has(id))
    const visibleDefs  = defOrder.filter(id => !matched.has(id))
    const rows = visibleTerms.length || 1

    return (
        // Use absolute positioning so we're not constrained by parent min-h-screen
        <div className="fixed inset-0 flex flex-col p-3 md:p-4 font-mono text-white overflow-hidden bg-transparent">
            ...
            <div className="fixed inset-0 pointer-events-none z-[200]">
                <AnimatePresence>
                    {particles.map(p => (
                        <motion.div key={p.id}
                            style={{ position: 'absolute', top: 0, left: 0, width: p.size, height: p.size,
                                borderRadius: '50%', backgroundColor: p.color,
                                boxShadow: `0 0 ${p.size * 2}px ${p.color}`,
                                transform: 'translate(-50%,-50%)' }}
                            initial={{ x: p.x, y: p.y, opacity: 1, scale: 1 }}
                            animate={{ x: p.x + p.vx, y: p.y + p.vy + 50, opacity: 0, scale: 0 }}
                            exit={{}}
            ...
            <div className="flex items-center justify-between shrink-0 mb-2">
                ...
                <div className="flex flex-col items-end leading-none">
                    <motion.span
                        key={score}
                        initial={{ scale: 1.15, color: score === scoreRef.current ? '#facc15' : '#facc15' }}
                        animate={{ scale: 1 }}
            ...
            <div className="shrink-0 mb-2">
                <div className="flex justify-between text-[10px] text-gray-500 mb-0.5">
                    <span>Pozostało: <b className="text-white">{remaining}</b>/{total}</span>
                        ...
                        {visibleTerms.map(id => {
                            ...
                            return (
                                <motion.div
                                    ...
                                    className={[
                                        'flex items-center justify-center px-3 rounded-xl border-2',
                                        'text-center text-xs md:text-sm font-bold select-none cursor-pointer',
                                        'transition-colors leading-tight overflow-hidden',
                                        sel   ? 'bg-secondary/25 border-secondary text-white shadow-[0_0_16px_rgba(99,102,241,0.45)]'
                                              : wrong
